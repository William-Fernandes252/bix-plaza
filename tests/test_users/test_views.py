...
class TestUserViewSet(ViewSetTest):
    ...
    class CaseAdmin:
        class DescribeList(
            UsesGetMethod, UsesListEndpoint, Returns200, AsUser("admin")  # type: ignore
        ):
            users = lambda_fixture(
                lambda db, faker: [
                    UserFactory.create(email=faker.email()) for _ in range(3)
                ]
            )

            def it_returns_all_users(
                self, users: list[User], results: list[dict[str, Any]]
            ):
        ...
        class DescribeCreate(
            UsesPostMethod, UsesListEndpoint, Returns201, AsUser("admin")  # type: ignore
        ):
            data = static_fixture(
                {
            ...
            def test_it_creates_a_new_user(
                self, initial_user_ids: set[str], json: dict[str, Any]
            ):
            ...
            def test_it_sets_expected_attrs(
                self, data: dict[str, Any], json: dict[str, Any]
            ):
        ...
        class DescribeRetrieve(
            UsesGetMethod, UsesDetailEndpoint, Returns200, AsUser("admin")  # type: ignore
        ):
            def test_it_returns_the_user(self, user: User, json: dict[str, Any]):
                expected = express_user_for_admin(user)
                actual = json
                assert actual == expected

        class DescribeUpdate(
            UsesPatchMethod, UsesDetailEndpoint, Returns200, AsUser("admin")  # type: ignore
        ):
            data = lambda_fixture(lambda faker: {"name": faker.name()})

            def test_it_sets_expected_attrs(self, data: dict[str, Any], user: User):
                user.refresh_from_db()
                expected = data
                assert_model_attrs(user, expected)

            def test_it_returns_key_value(self, user: User, json: dict[str, Any]):
                user.refresh_from_db()
                expected = express_user_for_admin(user)
                actual = json
        ...
        class DescribeDestroy(
            UsesDeleteMethod, UsesDetailEndpoint, Returns204, AsUser("admin")  # type: ignore
        ):
            initial_user_ids = precondition_fixture(
                lambda db: {
                    str(uuid) for uuid in User.objects.values_list("id", flat=True)
            ...
            def test_it_deletes_user(self, user: User, initial_user_ids: set[str]):
                expected = initial_user_ids - {str(user.pk)}
                actual = set(User.objects.values_list("id", flat=True))
                assert expected == actual

    class CaseAuthenticated:
        class DescribeList(
            UsesGetMethod, UsesListEndpoint, Returns403, AsUser("user")  # type: ignore
        ):
            """Users list is not allowed for authenticated users."""

        class DescribeRetrieve(
            UsesGetMethod, UsesDetailEndpoint, Returns200, AsUser("user")  # type: ignore
        ):
            def test_it_returns_the_user(self, user: User, json: dict[str, Any]):
        ...
        class DescribeUpdate(
            UsesPatchMethod, UsesDetailEndpoint, Returns200, AsUser("user")  # type: ignore
        ):
            data = lambda_fixture(lambda faker: {"name": faker.name()})
        ...
        class DescribeDestroy(
            UsesDeleteMethod, UsesDetailEndpoint, Returns204, AsUser("user")  # type: ignore
        ):
            initial_user_ids = precondition_fixture(
                lambda db: {
                    str(uuid) for uuid in User.objects.values_list("id", flat=True)
    ...
    class CaseAnonymous:
        class DescribeCreate(UsesPostMethod, UsesListEndpoint, Returns201):
            data = static_fixture(
                {
                    "email": "william.fernandes@bix.com",
                    "password": "123456",
            ...
            @pytest.fixture
            def client(self, client: Client, db):
                client.logout()
                return client
        ...
        class DescribeRetrieve(
            UsesGetMethod, UsesDetailEndpoint, Returns401  # type: ignore
        ): ...

        class DescribeUpdate(
            UsesPatchMethod, UsesDetailEndpoint, Returns401  # type: ignore
        ): ...

        class DescribeDestroy(
            UsesDeleteMethod, UsesDetailEndpoint, Returns401  # type: ignore
        ): ...
